...
export default function ProjectCard({ project }: { project: Project }) {
  const cardClass = project.featured
    ? "flex flex-col rounded-lg border bg-surface p-6 md:p-7 transition-colors border-signal/40 md:col-span-2 bg-gradient-to-b from-surface to-surface2"
    : "flex flex-col rounded-lg border bg-surface p-6 md:p-7 transition-colors border-border hover:border-borderHover";
  ...
  return (
    ...
    >
      {project.featured && (
        <span className="inline-flex w-fit items-center gap-1.5 rounded-full border border-signal/30 bg-signal/10 px-2.5 py-1 text-xs text-signal mb-4">
      ...
      <div className="mb-5">
        ...
        <div className="flex flex-wrap gap-2">
          {project.tech.map((t) => (
            <span
              key={t}
              className="rounded border border-border bg-surface2 px-2 py-1 text-xs font-mono text-muted"
            >
              {t}
            </span>
          ))}
